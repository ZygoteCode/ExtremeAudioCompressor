...
public class Program
{
    ...
    {
        Console.Title = "ExtremeAudioCompressor | Made by https://github.com/ZygoteCode/";
        ...
        {
            string fullPath = Path.GetFullPath("outputs") + "\\" + Path.GetFileNameWithoutExtension(file) + " (" + Path.GetExtension(file).ToUpper().Substring(1) + ").ogg";
            RunFFMpeg($"-threads {Environment.ProcessorCount} -i \"{file}\" -vn -ar 44100 -ac 1 -b:a 96k -map a \"{fullPath}\"");
        }
